...
const SubscriptionsPage = () => {
    ...
    const { isLoading } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await axios.get("/api/subscriptionsProducts")
                console.log(res)
    ...
    if (isLoading) { return <div className="flex-1 p-4">Chargement...</div> }

    return (
        <div className="flex-1 p-4">
            <h1 className="text-xl font-bold mb-4">{t("Subscriptions")}</h1>
            {orders?.length === 0 ? (
